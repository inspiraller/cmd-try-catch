...
describe('promiseExec - all', () => {
  describe('promiseExec', () => {
    ...
    it('should return promise {error: value}', async () => {
      const response: IObjSuccessOrErrorVoid = await promiseExec({
        cmd: 'cmdThatDoesntExist arg'
      }).catch(err => {
  ...
  describe('handleExecOut', () => {
    it('should return {success: value}', async () => {
      const response: IObjSuccessOrErrorVoid = await new Promise(
        (resolve: TResolveFunc, reject) => {
